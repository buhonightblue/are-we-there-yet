<script lang="ts">
	...
	import { byEarliestDate, latestEpisode, notYetReleased } from '$lib/utils';
	import Localized from '$lib/Lang/Localized.svelte';
	import type { Show, AccentColors } from '$lib/utils';
...
>
	<div class="hidden sm:block w-24 mt-3 ml-3 mb-8 relative flex-none">
		<img class="z-0 rounded-md" src={show.poster} alt="Poster for {show.name}" />
		...
		{#if expanded}
			<div class="grid grid-cols-2 justify-between">
				{#each unfinishedEpisodes.slice(0, 4) as episode}
					<Episode {episode} status={show.status} latest={episode.id === latest.id} inList={true} />
				{/each}
			</div>
			<button
			...
			>
				...
				{:else}
					<Localized key="UNIT_SHOW_LESS_PLURAL" fallbackKey="UNIT_SHOW_LESS" />
				{/if}
			</button>
		{:else}
